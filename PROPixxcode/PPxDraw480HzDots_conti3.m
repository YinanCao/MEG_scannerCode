function PPxDraw480HzDots_conti3
...
for on = 1:10
   for off = 1:10
    cycle = [ones(1,on),zeros(1,off)];
    NN = length(cycle);
    ff(count) = 1./(NN*(ifi/4));
    sig = repmat(cycle,1,nc)';
    sig(1:4*floor(length(sig)/4));
    x = reshape(sig,[4,floor(length(sig)/4)]);
    x = x(:,1:50);
    contrast_table(:,:,count) = x;
    count = count + 1;
   end
end
id = [];
...
ff(id)
contrast_table = contrast_table(:,:,id(ff(id)>0));

for trl = 1:5

    for block = 1:size(contrast_table,3)

    OnsetTime = GetSecs;
    
    for f = 1:size(contrast_table,2)
        colour = contrast_table(:,f,block);
        for quadrant = 1:4
